...
test('mobile shell uses a burger menu instead of the bottom nav bar', () => {
  ...
  assert.match(sidebarNav, /href:\s*"\/chat-exercises"/);
  assert.match(sidebarNav, /label:\s*"Chat Exercises"/);
  assert.match(mobileMenu, /href:\s*"\/chat-exercises"/);
  assert.match(mobileMenu, /label:\s*"Chat Exercises"/);
});
